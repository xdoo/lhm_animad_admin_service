...
@ExposesResourceFor(Zookeeper_.class)
@RequestMapping("/zookeepers/search")
public class Zookeeper_SearchController {
		
    @Autowired
    QueryService service;

    @Autowired
    Zookeeper_Repository repository;
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
	    ...
	    Class tmpClass = Zookeeper_.class;
	    while (tmpClass != null) {
        ...
        Stream<Zookeeper_> zookeeperStream;
        try {
            zookeeperStream = service.query(q, Zookeeper_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            zookeeperStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = zookeeperStream.map(assembler::toResource).collect(Collectors.toList());
